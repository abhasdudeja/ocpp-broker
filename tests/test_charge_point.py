...
async def test_boot_notification_registers_charger():
    ...
    assert isinstance(result, call_result.BootNotificationPayload)
...
async def test_authorize_uses_tag_manager_response():
    ...
    assert isinstance(result, call_result.AuthorizePayload)
...
async def test_start_transaction_returns_generated_id_and_tag_info():
    ...
    assert isinstance(result, call_result.StartTransactionPayload)
